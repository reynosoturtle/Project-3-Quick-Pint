...
class BarsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create bar" do
    assert_difference('Bar.count') do
      post bars_url, params: { bar: { business_hours: @bar.business_hours, images: @bar.images, name: @bar.name } }
    end
  ...
  test "should update bar" do
    patch bar_url(@bar), params: { bar: { business_hours: @bar.business_hours, images: @bar.images, name: @bar.name } }
